...
module.exports = {
    ...
    postComment(req, res) {
        ...
        fs.readFile(filePath, 'utf8', (err, data) => {
            ...
            // comment id도 그냥 막 늘리는게 나을지, 아니면 postid 마다 따로 생각하는게 나을지
            jsonData.commentId = (jsonDataArray.items.length + 1).toString();
            jsonDataArray.items.push(jsonData);
    ...
    putComment(req, res) {
        ...
        fs.readFile(filePath, 'utf8', (err, data) => {
            ...
            const targetPost = jsonDataArray.items.find(item => item.postId == req.params.postId && item.commentId == req.params.commentId);

            if(targetPost) {
                targetPost.comment = jsonData.comment;
                targetPost.time = jsonData.time;
            } else {
                console.log('수정할 댓글 찾을 수 없음');
                return res.status(404).json({ error: '수정할 댓글 찾을 수 없음' });
            }
            ...
            fs.writeFile(filePath, updatedJsonData, 'utf8', writeErr => {
                ...
                res.json({ message: '데이터가 수정 되었습니다.' });
            })
        });
    },
